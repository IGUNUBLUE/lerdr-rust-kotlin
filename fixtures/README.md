...
no trailing whitespace. Generated by test-only exporters in the reference
repo (`~/Projects/lerdr`, files named `*_export_test.go`, gated behind
`LERDR_EXPORT_FIXTURES=1`) or frontend export scripts. **Regenerate, never
hand-edit.** A vector that fails means the implementation is wrong or the
generator drifted — fix at the source, not in the fixture.
