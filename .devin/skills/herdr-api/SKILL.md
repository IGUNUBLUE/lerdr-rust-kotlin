...
- High-value methods beyond the relay's current subset: `layout.apply/export` (templates), `command.invoke` (endpoint-issued ids, revision-validated), `agent.explain` (detection debugging), `notification.show`, `plugin.action.invoke` (drive other plugins), `server.reload_config`, `integration.*` — full table in `docs/11-stack-practices.md`.
...
- `relay/crates/lerdr-herdr/` — the socket client implementation.
